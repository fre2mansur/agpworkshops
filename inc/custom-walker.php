...
if (! class_exists ( 'custom_WP_Bootstrap_Navwalker' )) :
...
class mobile_Navwalker extends Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		$output .= "\n$indent<div id=\"collapseOne\" class=\"collapseDropDown\"><ul class=\" navbar-nav dropdown-menu mx-auto text-capital text-center\" role=\"menu\">\n";
	}
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
			...
			*/
			...
			if ( $args->has_children && $depth === 0 ) {
				$atts['href']        = '#';
                $atts['data-toggle'] = 'collapseDropDown';
                $atts['data-target'] = '.collapseDropDown';
                $atts['data-text'] = 'CollapseDropDown';
				$atts['class']       = 'nav-link side-arrow';
			} else {
			...
			if ( ! empty( $icon ) ) {
				...
				$item_output .= '<a' . $attributes . '><span data-toggle="collapseDropDown" data-target=".collapseDropDown" data-text="CollapseDropDown">';
			}
			$item_output .= $args->link_before . apply_filters( 'the_title', $item->title,
					$item->ID ) . $args->link_after;
			$item_output .= ( $args->has_children && 0 === $depth ) ? ' </span></a>' : '</a>';
