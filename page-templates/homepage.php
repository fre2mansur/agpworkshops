...
<!--Gallery Starts-->
        ...
        //   'post_status' => 'publish' );
		  ...
		  $today = date('Ymd');
				$workshops = $wpdb->get_results("SELECT * FROM $wpdb->postmeta INNER JOIN $wpdb->post ON ($wpdb->post.ID = $wpdb->postmeta.post_id)  WHERE meta_key LIKE 'start_date_wp' AND (meta_value > '$today' or meta_value = '$today' AND $wpdb->post post_status = 'publish') ORDER BY meta_value ASC LIMIT 9" );
		  		$postStartDate = null;
